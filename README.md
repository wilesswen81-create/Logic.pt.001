...
class MapSystem:
    ...
    def get_travel_time(start_node, end_node):
        ...
        pos1 = LOCATIONS_POS.get(start_node)
        pos2 = LOCATIONS_POS.get(end_node)
        
        if not pos1 or not pos2:
            return 10 # ค่า Default ถ้าหาที่ตั้งไม่เจอ
            
    # คำนวณระยะทางแบบบล็อกเมือง (Manhattan Distance)ใช้ สูตร: |x1 - x2| + |y1 - y2|
        distance = abs(pos1[0] - pos2[0]) + abs(pos1[1] - pos2[1])
        
    # ปรับตัวคูณเพื่อให้เวลาดูสมจริง เช่น ถ้าห่างกัน 5 หน่วย * 1.5 = ใช้เวลา 7-8 
        travel_time = int(distance * 1.5)
        
    # กำหนดขั้นต่ำว่าถ้าเดินทางข้ามที่ ยังไงก็ต้องใช้เวลาอย่างน้อย 2 
        return max(2, travel_time)
