...
class EducationSystem:
    ...
    TRACK_BENEFITS = {
        "Science-Mathematics": ["Science", "Engineering", "Health"],
        "Arts-Language": ["Arts", "Business", "Education", "Sports"]
    }

    @staticmethod
    def get_study_progress(player, major_name, current_presses, required_presses):
    ##  แสดงผล Progress: 'เรียนแล้ว/ต้องเรียน (จากเดิม)' ##
    
     # บัฟมีผลเฉพาะเทิร์น 1-6 (มัธยม)
        if player.turn <= 6:
            discount = player.study_discounts.get(major_name, 0)
            final_required = max(1, required_presses - discount)
            
            if discount > 0:
            # กรณีมีบัฟ (ตรงสาย)
                return f"{current_presses}/{final_required} (จาก {required_presses})"
            else:
            # กรณีไม่มีบัฟ (ไม่ตรงสาย)
                return f"{current_presses}/{required_presses} (จาก {required_presses})"
        
      # มหาลัยใช้ค่าปกติ
        return f"{current_presses}/{required_presses}"
...
class MapSystem:
    ...
    def get_travel_time(start_node, end_node):
     ...
     # 4. แปลงเป็นเวลา (Time Unit) ใช้ตัวคูณเดิมเพื่อให้เลขดูสมจริงจร้า
...
class EndGameSystem:
    ...
    @staticmethod
    def get_rank(value):
     ## สำหรับค่า Status ปกติ (0-100) ##
        if value >= 81: return "S"
        if value >= 61: return "A"
        if value >= 41: return "B"
        if value >= 21: return "C"
        return "D"

    @staticmethod
    def get_money_rank(money):
     ## สำหรับค่าเงินโดยเฉพาะ (Hardcore Logic) พวกมุงตุยหมดแน่ กุเก็บหมด ##
        if money >= 500000:
            return "S"   # หลักแสนปลาย - ล้าน
        elif money >= 100000:
            return "A"   # หลักแสน
        elif money >= 10000:
            return "B"   # หลักหมื่น
            ...
            return "D"   # หลักร้อย (0-999)
        else:
            return "F"   # ติดลบ (เป็นหนี้!!! แกไม่รอกแน่! 55555)

    @staticmethod
    def generate_player_cards(player):
        cards = []
        
     # 1. ความสุข (100 - Stress)
        rank_h = EndGameSystem.get_rank(100 - player.stats["stress"])
        cards.append({"category": "🌈 ความสุข", "content": random.choice(END_DATA["stress"][rank_h])})
        
     # 2. การเงิน (Hardcore Check)
        money_val = player.stats["money"]
        rank_m = EndGameSystem.get_money_rank(money_val)
        cards.append({"category": "💰 การเงิน", "content": random.choice(END_DATA["money"][rank_m])})
        
     # 3. ด้านอาชีพ (Knowledge 0-100)
        rank_c = EndGameSystem.get_rank(player.stats["knowledge"])
        cards.append({"category": "🎓 อาชีพ", "content": random.choice(END_DATA["career"][rank_c])})
        
     # 4. ด้านสุขภาพ (Health 0-100)
        rank_he = EndGameSystem.get_rank(player.stats["health"])
        cards.append({"category": "💪 สุขภาพ", "content": random.choice(END_DATA["health"][rank_he])})

     # 5. เวลาชีวิต
        cards.append({"category": "⌛ เวลาชีวิต", "content": random.choice(END_DATA["time"]["Final"])})
        
        return cards
        
    ## เปิดการ์ดเอนเกม 5 ใบของตัวเอง ทีละคร ##
class RecapController:

    def __init__(self, player_list):
        self.players = player_list
        self.current_p_idx = 0  # ลำดับผู้เล่น (0, 1, 2...)
        self.current_c_idx = 0  # ลำดับการ์ด (0-4)
        
     # เตรียมข้อมูลการ์ดของทุกคนไว้ล่วงหน้า
        self.all_player_cards = [EndGameSystem.generate_player_cards(p) for p in player_list]
        self.is_finished = False

    def get_current_card(self):
     ## ดึงข้อมูลการ์ดที่ต้องโชว์บนหน้าจอตอนนี้ ##
        if self.is_finished:
            return None
        
        player_name = self.players[self.current_p_idx].name
        card_data = self.all_player_cards[self.current_p_idx][self.current_c_idx]
        
        return {
            "player_name": player_name,
            "category": card_data["category"],
            "content": card_data["content"],
            "progress": f"การ์ดใบที่ {self.current_c_idx + 1}/5"
        }

    def next_step(self):
     ## ฟังก์ชันสำหรับกดปุ่ม 'ถัดไป' ในตอนอ่านการ์ด ##
        if self.is_finished: return
        
     # เพิ่มลำดับการ์ด
        self.current_c_idx += 1
        
     # ถ้าดูการ์ดของคนนี้ครบ 5 ใบแล้ว
        if self.current_c_idx >= 5:
            self.current_c_idx = 0 # กลับไปเริ่มใบที่ 1 ใหม่
            self.current_p_idx += 1 # เปลี่ยนเป็นคนถัดไป
            
         # ถ้าเวียนครบทุกคนแล้ว
            if self.current_p_idx >= len(self.players):
                self.is_finished = True
                return "END_SHOW" # ส่งสัญญาณว่าจบการโชว์ทั้งหมดแล้ว
            
            return "NEXT_PLAYER" # ส่งสัญญาณว่าเปลี่ยนคนดู
        
        return "NEXT_CARD" # ส่งสัญญาณว่าโชว์ใบถัดไป
...
#### JOB DATA( For พรีเกม,เอดูเคชัน,ยูนิ ) ####
JOB_DATA = {
    "Health": {
    #ทำงานที่รพ.หมดเลย
    ...
    ##  ด้านการเงิน (Money) 
        {
            ...
            "choices": [
                ...
                {"text": "อยู่เฉยๆ ทำงานวนไป", "result_msg": "รอดตัวไปนะ เหรียญนั่นกลายเป็น 0 ในวันถัดมา (Stress -5)", "effects": {"stress": -5}}
            ]
        },
        {
            "title": "ซ่อมบ้านด่วน",
     ...
     ##  ด้านสุขภาพ (Health) 
        ...
        },
        {
            ...
            "choices": [
                {"text": "จัดหนัก เยียวยาจิตใจ", "result_msg": "หายเครียด...แต่คอเลสเตอรอลพุ่ง! (Stress -20, Health -10, Money -599)", "effects": {"stress": -20, "health": -10, "money": -599}},
                {"text": "กลับไปกินสลัดที่บ้าน", "result_msg": "ทนหิวแปปนึง...แต่สุขภาพดี! (Health +10, Stress +5)", "effects": {"health": 10, "stress": 5}}
            ]
        },
    ...
    ##  ด้านการงาน/ความรู้ (Career & Knowledge) 
        {
            ...
            "choices": [
                ...
                {"text": "หาดูฟรีใน YouTube เอา", "result_msg": "ได้ความรู้บ้างกระปิดกระปอย (Knowledge +5)", "effects": {"knowledge": 5}}
            ]
        },
        {
            ...
            "choices": [
                ...
                {"text": "แกล้งเนียนทำเป็นไม่รู้", "result_msg": "บรึ้มม!เรื่องแดงขึ้นมาทีหลัง โดนหักโบนัส! (Money -2000, Stress +30)", "effects": {"money": -2000, "stress": 30}}
            ]
        },
    ...
    ##  ด้านสังคม/ความสัมพันธ์ (Social)
        {
            ...
            "choices": [
                {"text": "ใส่ตามมารยาททุกงาน", "result_msg": "สังคมดีเด่น...แต่กินมาม่า (Money -2000, Stress -5)", "effects": {"money": -2000, "stress": -5}},
                {"text": "เลือกไปเฉพาะงานสำคัญ", "result_msg": "เลือกแค่ญาติสนิท พอประหยัดเงินไปได้บ้าง (Money -500, Stress +5)", "effects": {"money": -500, "stress": 5}}
            ]
        },
        {
            ...
            "choices": [
                {"text": "เข้าข้างฝ่ายที่ดูมีเหตุผล", "result_msg": "ได้มิตรเพิ่ม 1 แต่ได้ศัตรูเพิ่มอีก 1 นะ... (Stress +10)", "effects": {"stress": 10}},
    ...
    ##  ด้านโชคลาภ (Luck/Random) 
        {
            ...
            "choices": [
                {"text": "กดลิงก์ไปรับรางวัล", "result_msg": "เริ่ดด!!โดนแก๊งคอลเซ็นเตอร์ดูดเงิน... (Money -5000, Stress +40)", "effects": {"money": -5000, "stress": 40}},
                {"text": "ลบทิ้งไป มิจฉาชีพชัวร์", "result_msg": "แหมม...คนสมัยนี้ฉลาดมาก รอดตัวไปนะ (Knowledge +2)", "effects": {"knowledge": 2}}
            ]
        },
        {
            ...
            "choices": [
                {"text": "เหมามาเลย เผื่อรวย!", "result_msg": "ถูกกินตามระเบียบครับพี่น้อง... (Money -600, Stress +5)", "effects": {"money": -600, "stress": 5}},
                {"text": "เดินหนีอย่างใจแข็ง", "result_msg": "เมื่อคุณเอาชนะความโลภได้รางวัลคือ! ประหยัดเงินไปได้ (Money +0)", "effects": {}}
            ]
        }
    ]
}
...
END_DATA = {
    ...
    "money": {
        "S": ["คุณกลายเป็นเศรษฐีที่มีอิสรภาพทางการเงินสมบูรณ์แบบ ชีวิตล้อมรอบด้วยความหรูหรา", 
        "บัญชีธนาคารมีเลขศูนย์ยาวจนนับไม่ถ้วน เนรมิตได้ทุกอย่างด้วยพลังเงิน", 
        "การวางแผนการเงินยอดเยี่ยมทำให้คุณก้าวสู่ระดับสูงสุดของสังคมอย่างสง่างาม"],
        
        "A": ["คุณมีความมั่งคั่งในระดับที่น่าอิจฉา สามารถใช้ชีวิตได้ตามใจปรารถนาโดยไม่ต้องกังวล",
        "ฐานะการเงินแข็งแกร่งมาก คุณมีทรัพย์สินมากพอที่จะรองรับความฝันใหญ่ๆ ได้สบาย", 
        "คุณบริหารเงินเก่งจนมีเหลือกินเหลือใช้ และยังมีปัญญาแบ่งปันให้คนรอบข้างได้ด้วย"],
        
        "B": ["คุณมีชีวิตที่มั่นคง มีเงินเก็บพอประมาณสำหรับการใช้ชีวิตที่เรียบง่ายแต่ไม่ลำบาก",
        "ฐานะการเงินปานกลาง ไม่ขัดสนแต่ยังต้องวางแผนใช้จ่ายอยู่เสมอ เป็นชีวิตที่พอดี",
        "แม้ไม่รวยล้นฟ้า แต่คุณบริหารเงินได้ดีพอที่จะมีบ้านที่อบอุ่นและอาหารดีทุกมื้อ"],
        
        "C": ["รายได้พอกับรายจ่ายแบบเดือนชนเดือน คุณยังต้องระวังการใช้สอยมากกว่านี้",
        "คุณพอมีเงินประทังชีวิต แต่ความประมาทอาจทำให้คุณลำบากในระยะยาวได้",
        "การเงินอยู่ในเกณฑ์เสี่ยง คุณต้องใช้ความพยายามอย่างมากเพื่อให้พ้นขีดอันตราย"],
        
        "D": ["สถานะการเงินตึงเครียด ชีวิตวนเวียนกับการหมุนเงิน แต่ประสบการณ์นี้คือบทเรียนใหญ่",
        "คุณพบว่าความสุขชั่วคราวอาจทำลายฐานะได้ หากไม่เริ่มสร้างความมั่นคงตั้งแต่วันนี้",
        "เงินขัดสนทำให้ต้องใช้ชีวิตประหยัดที่สุด แต่มันสอนให้รู้ค่าของเงินทุกบาทที่หามาได้"]
        
        "F": ["สถานะทางการเงินของคุณเข้าขั้นวิกฤต หนี้สินมหาศาลกลายเป็นเงาตามตัวที่สลัดไม่หลุด คุณอาจต้องใช้เวลาทั้งชีวิตที่เหลือเพื่อชดใช้มัน",
        "ตัวเลขติดลบในบัญชีบอกเล่าถึงการตัดสินใจที่ผิดพลาดมหาศาล คุณกำลังยืนอยู่บนขอบเหวทางการเงินและต้องเริ่มต้นใหม่จากติดลบ",
        "ชีวิตของคุณถูกพันธนาการด้วยหนี้สินรุงรัง การดิ้นรนหาทางออกดูเหมือนจะมืดแปดด้าน แต่บทเรียนที่เจ็บปวดนี้จะเตือนใจคุณไปตลอดกาล"
]
    },
    
    
    "stress": { # (คะแนน 100 - Stress)
        "S": ["คุณคือปรมาจารย์ด้านการจัดการอารมณ์ จิตใจสงบนิ่งเหมือนน้ำ ชีวิตรื่นรมย์เสมอ",
        "ความสุขภายในเบ่งบานเต็มที่ คุณปล่อยวางและมองโลกในแง่ดีจนใครๆ ก็อยากอยู่ใกล้",
        "ไม่มีพายุใดทำลายความสงบในใจคุณได้ คุณผ่านบททดสอบมาด้วยรอยยิ้มที่แข็งแกร่ง"],
        
        "A": ["คุณมีความสุขกับชีวิตเป็นส่วนใหญ่ จัดการปัญหาได้ดีและยิ้มได้กว้างในทุกๆ วัน",
        "จิตใจแจ่มใสและมีพลังบวกล้นเหลือ คุณรู้วิธีตัดความกังวลออกไปจากชีวิตได้รวดเร็ว",
        "ชีวิตเต็มไปด้วยเสียงหัวเราะ คุณเป็นคนที่ดูดซับความทุกข์ได้น้อยและขยายความสุขได้มาก"],
        
        "B": ["คุณมีความเครียดบ้างตามประสาคนสู้ชีวิต แต่ยังรักษาสมดุลของจิตใจไว้ได้ค่อนข้างดี",
        "บางวันยิ้มได้ บางวันอ่อนแอ แต่นั่นคือรสชาติมนุษย์ คุณจัดการมันได้เก่งพอตัว",
        "ชีวิตมีทั้งหยาดน้ำตาและเสียงหัวเราะ คุณเรียนรู้ที่จะอยู่กับความเครียดอย่างเป็นมิตร"],
        
        "C": ["ความกังวลเริ่มเกาะกินใจบ่อยขึ้น คุณควรหาเวลาพักผ่อนเพื่อเติมพลังบวกบ้างนะ",
        "คุณแบกภาระจนรอยยิ้มเริ่มจางหายไป ความเครียดเริ่มส่งผลต่อการตัดสินใจของคุณ",
        "ใจของคุณว้าวุ่นและเหนื่อยง่ายกว่าปกติ ลองหยุดวิ่งแล้วนั่งพักดูบ้างอาจจะดีขึ้น"],
        
        "D": ["ชีวิตเต็มไปด้วยความกดดันมหาศาล สภาพจิตใจเหนื่อยล้าบอกให้รู้ว่าคุณต้องพักด่วน",
        "คุณแบกโลกทั้งใบไว้จนแทบหยุดหายใจ ความเครียดทำให้คุณลืมวิธีมีความสุขง่ายๆ",
        "ร่องรอยความเหนื่อยปรากฏชัดในแววตา คุณสู้หนักเกินไปจนละเลยหัวใจของตัวเอง"]
    },
    
    
    "career": {
        "S": ["คุณประสบความสำเร็จสูงสุดในอาชีพฝัน เป็นผู้เชี่ยวชาญชื่อดังและทำสิ่งที่รักทุกวัน",
        "หน้าที่การงานเติบโตสู่จุดสูงสุด เป็นแรงบันดาลใจและได้รับการยอมรับกว้างขวาง",
        "คุณไม่ได้แค่ทำงาน แต่คุณสร้างตำนาน ทุกหยาดเหงื่อเปลี่ยนเป็นความสำเร็จที่ยิ่งใหญ่"],
        
        "A": ["คุณมีหน้าที่การงานที่มั่นคงและรุ่งเรือง ก้าวหน้าไปไกลกว่าเพื่อนร่วมรุ่นหลายคน",
        "ความรู้ที่คุณสะสมมาทำให้คุณเป็นคนสำคัญในสายงาน และมีอนาคตที่สดใสรออยู่",
        "คุณทำงานได้ยอดเยี่ยมและมีความสุขกับความสำเร็จที่ได้มาจากหยาดเหงื่อแรงกาย"],
        
        "B": ["คุณทำงานได้ตามมาตรฐานและเลี้ยงตัวได้ดี มีโอกาสเติบโตหากตั้งใจพัฒนาต่อ",
        "อาชีพการงานอยู่ในระดับที่น่าพอใจ คุณมีความเชี่ยวชาญเฉพาะทางระดับหนึ่งแล้ว",
        "ชีวิตการทำงานราบรื่นดี แม้ไม่โดดเด่นที่สุดแต่คุณก็มีความสุขกับสิ่งที่ทำอยู่"],
        
        "C": ["งานของคุณยังติดขัดและไม่ก้าวหน้าเท่าที่ควร ลองหาความรู้เพิ่มเติมเพื่อเปิดโอกาสใหม่",
        "คุณยังค้นหาตัวเองไม่เจอในงานที่ทำอยู่ ทำให้ขาดแรงจูงใจในการพัฒนาศักยภาพ",
        "ความพยายามในการทำงานยังมีน้อยไปนิด ทำให้คุณยังก้าวไปได้ไม่ไกลนักในสายงาน"],
        
        "D": ["แม้ยังไม่อยู่ในจุดที่หวัง แต่ความพยายามที่ผ่านมาคือบทเรียนล้ำค่าที่รอวันงอกงาม",
        "เส้นทางอาชีพอาจขรุขระ แต่นั่นคือจุดเริ่มต้นของการค้นหาตัวเองที่ยิ่งใหญ่กว่าเดิม",
        "คุณอาจยังไม่เจอสิ่งที่ใช่ แต่การไม่หยุดก้าวเดินคือความสำเร็จที่น่าชื่นชมของนักสู้"]
    },


    "health": {
        "S": ["ร่างกายแข็งแกร่งสดใสเหมือนวัยรุ่น มีพลังงานเหลือล้นในการใช้ชีวิตไร้ขีดจำกัด",
        "ดูแลตัวเองดีเยี่ยม สุขภาพที่ฟิตเปรี๊ยะกลายเป็นสมบัติล้ำค่าให้คุณทำได้ทุกอย่าง",
        "ความแข็งแรงของกายส่งผลให้ใจสดชื่น พร้อมเผชิญทุกอุปสรรคด้วยสภาพที่สมบูรณ์"],
        
        "A": ["คุณสุขภาพดีมากและมีระเบียบวินัยในการดูแลตัวเอง ทำให้โรคภัยเข้าหาได้ยาก",
        "ร่างกายกระปรี้กระเปร่าและพร้อมสำหรับกิจกรรมท้าทาย คุณดูดีและแข็งแรงกว่าใคร",
        "การลงทุนกับสุขภาพให้ผลตอบแทนคุ้มค่า คุณมีอายุยืนยาวและคุณภาพชีวิตที่ยอดเยี่ยม"],
        
        "B": ["สุขภาพอยู่ในเกณฑ์ปกติ มีเจ็บป่วยบ้างตามฤดูกาลแต่โดยรวมยังแข็งแรงดี",
        "คุณดูแลร่างกายได้ปานกลาง แม้ไม่ฟิตจัดแต่ก็ไม่มีโรคประจำตัวร้ายแรงมากวนใจ",
        "ร่างกายยังทำงานได้ดี แต่อย่าลืมหาเวลาออกกำลังกายเพิ่มเพื่อให้แข็งแรงกว่าเดิม"],
        
        "C": ["ร่างกายเริ่มส่งสัญญาณเตือนว่าต้องพัก คุณเริ่มเหนื่อยง่ายและเจ็บออดๆ แอดๆ",
        "การละเลยสุขภาพเริ่มส่งผลเสีย คุณควรลดการกินตามใจปากและเริ่มออกกำลังกายบ้าง",
        "ภูมิต้านทานต่ำลงทำให้คุณป่วยบ่อยขึ้น ถึงเวลาต้องหันกลับมาดูแลเครื่องจักรนี้แล้ว"],
        
        "D": ["สุขภาพทรุดโทรมจากการทำงานหนักเกินไป เตือนว่าไม่มีอะไรสำคัญกว่าการดูแลตัวเอง",
        "คุณใช้ร่างกายหนักจนมันประท้วง ความอ่อนแอขอกลับมาใส่ใจตัวเองบ้างก่อนจะสายไป",
        "ร่างกายเหมือนเครื่องจักรขาดการบำรุงรักษา ถึงเวลาหยุดพักเพื่อซ่อมแซมส่วนสึกหรอ"]
    },
    ...
    "time": {
        "Final": [
            "⌛ เวลาชีวิต: คุณได้ใช้เวลาชีวิตมาอย่างคุ้มค่ามาก ให้ลองทบทวนดูว่ามีอะไรที่อยากลองทำเพิ่ม หรืออยากลองเปลี่ยนแปลงการกระทำดูอีกครั้งไหมในชีวิตของคุณหลังจากนี้",
            "⏳ เวลาชีวิต: ทุกวินาทีที่คุณเลือกเดินทางมาถึงจุดนี้คือสิ่งที่คุณตัดสินใจเองได้ดีที่สุดแล้ว ลองถามตัวเองดูว่าหากมีโอกาสอีกครั้ง คุณจะยังเดินบนเส้นทางเดิมหรือจะลองสร้างตำนานบทใหม่",
            "🕰️ เวลาชีวิต: เวลาที่ผ่านไปไม่มีวันย้อนกลับ แต่ประสบการณ์ที่ได้รับจะอยู่กับคุณตลอดไป ลองมองย้อนกลับไปดูสิว่ามีบทเรียนไหนที่อยากนำไปใช้ในการเดินทางครั้งหน้าบ้าง"
        ]
    }
}
